...
# 讀取網頁資料 https://ebus.gov.taipei/Route/StopsOfRoute?routeid="station_id"
url = f"https://ebus.gov.taipei/Route/StopsOfRoute?routeid={station_id}"
...
for tag in span_tags:
    # 清理檔案名稱
    file_name = tag.text.strip().replace("\n", "").replace("\r", "").replace(" ", "_")
    file_name = "".join(c for c in file_name if c.isalnum() or c in ("_", "-"))  # 保留字母、數字、底線和連字號

    if not file_name:  # 如果檔案名稱為空，跳過該標籤
        continue
...
if __name__ == "__main__":
    file_path = r'C:\\Users\\User\\Desktop\\CYCU_oop_11022329\\20250408\\P1\\0東.html'
